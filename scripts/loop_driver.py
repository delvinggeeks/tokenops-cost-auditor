...
def build_prompt(number: int, title: str, body: str, author_email: str) -> str:
    ...
    return (
        ...
        "CRITICAL — you run HEADLESS and are NOT re-invoked after you yield; any background "
        "or Monitor task you start is KILLED the instant your turn ends. Do the ENTIRE job in "
        "ONE synchronous pass: edit → run each `uv run ...` check in the FOREGROUND (block "
        "until it finishes, read its result) → commit → push → open the PR → label it — all "
        "before you stop. NEVER background a check and 'wait' for it, and NEVER stop before the "
        "PR exists. If you yield to wait, or stop before the PR is open, ALL your work is "
        "discarded and the issue is left stuck.\n\n"
