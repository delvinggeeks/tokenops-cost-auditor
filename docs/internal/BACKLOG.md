...
- **`/openapi.json` is unauthenticated and contradicts the admin 404 design (found 2026-08-01, needs a ruling — may be intentional).** `main.py` sets `docs_url=None, redoc_url=None` (Swagger/ReDoc off) but leaves `openapi_url` at its default, so `GET /openapi.json` serves **all 103 paths incl. 7 `/admin/*` routes and the `x-user-email` dev shim header**, verified at runtime. `routes_admin.py` deliberately returns **404, not 403**, on a wrong/unset admin token — obscurity by design — which publishing the admin paths undoes. Either gate/disable `openapi_url` or drop the 404 pretence; today the two choices cancel. Note the spec is NOT needed for docs: `scripts/export_openapi.py` emits Markdown to `docs-site/api/endpoints.md` and never writes a spec file.

- **API-first is a stated intention, not a property of the system (inventoried 2026-08-01).** 103 endpoints exist; **6 are reachable by an API client** (4 read + `/api/v1/ingest` + `/api/v1/collector/ship`). **There is no scoped write at all** — `api_scopes.py` states this deliberately, so every mutation (connect source, mint key, register webhook, invite member, change role, set alert threshold, rename/switch workspace, save view, purge, close account, finding feedback) is HTML form + session cookie, accepting neither JSON nor a bearer token. Sharpest case: **`/explore` already has a full filter model** (`date_from, date_to, group, tier, source_id, model, detector, severity, status`) that is **unreachable over JSON** — the capability is built, the API is not. Also absent: offset/cursor paging (`/audits` takes `limit` only; `/findings` is unbounded), bulk endpoints, an async job API, export over JSON, CORS (so no browser client can call it), and any verb but POST. `GET /audits/{id}/status` requires a SESSION, so an API client cannot poll its own audit — the JS SDK works around it by polling `listAudits()`.
- **The configuration plane does not exist (inventoried 2026-08-01) — this is the enterprise precondition.** Per-workspace, the entire configurable surface is **one boolean** (`Workspace.cohort_opt_in`) and one string (`Source.schedule`); four alert rules with one threshold each are the only user-editable thresholds in the product. Meanwhile **~40 detector thresholds are process-global env vars** (`d1_short_completion_t`, `d2_cache_min_repeats`, `d3_bloat_mult` …), so two enterprises on one deployment cannot be tuned differently. There is no `WorkspaceSettings` table, no policy table, no feature flags, no per-tenant pricing, no per-workspace detector toggle, no tag taxonomy. Enterprise customization is not missing features — **the dimension was never created**, and tenant rate cards, per-workspace thresholds and DECLARED intent all block on it.
- **docs/12 flywheel coverage map — what the recorded vision has vs what stands (measured 2026-08-01).** `PLAN.md` §0.0 defines six stages (INPUT→ANALYZE→REPORT→ACT→PREVENT→IMPROVE) and notes PREVENT includes *"later real-time control"*, so enforcement is sequenced, not forbidden. Built: T1 file, T2 account, T3 collector; 9 detectors; JSON/PDF/web + showback CSV; findings + feedback + verified savings; alerts + staged budgets; frame/cohort/benchmarks. **NOT built: T4 STREAM (OTLP) — `docs/13-T4-OTLP-SPEC.md` exists, zero OTLP code in `src/`; D7 — named in the INTENT LAW behavioural suite, never shipped; DECLARED intent — no `task_class`/`quality_sensitivity`/`expected_recurrence` anywhere, only a free-form `tag`, so the "you tell us why" half of the INTENT LAW has no channel; FOCUS-aligned export (R-STANDARDS).** T5 gateway is deliberately Phase 2. DECLARED intent is the one that blocks the advisory/model thesis — without it every recommendation reasons from shape alone, which the law explicitly forbids inferring intent from.
- **Proposed architecture frame (not adopted — needs a founder ruling): six lifecycle stages × three planes.** API plane (today only REPORT is exposed; benchmark against Stripe for paging/idempotency, OTel for ingest semantics, FOCUS for cost export — all three already named in R-STANDARDS) · Configuration plane (absent, the precondition) · Data plane (strong: counts-only, provenance, evidence rows). Makes "no gaps" auditable as a grid rather than aspirational.

- **Landing page + sub-pages: full rework, and a STANDING LAW that they track what shipped (founder 2026-08-01).** Today's copy both **over-claims** (the "your logs and prompts are never used to train any model" sentence, in EIGHT places incl. privacy policy, terms and the footer of every public page — contradicted by `flywheel/frame.py`, whose inclusion test is `benchmark_sharing is not False`, i.e. **silence means included**) and **badly under-sells** (nine shipped detectors each carrying dev-facing plain-language fixes with a dollar value; the Claude Code collector; verified savings re-measured after a fix — none of it told as the token-management story). Rework in three lanes: **rewrite** what is true and undersold · **remove/replace** the training claim, pending the ruling on whether the frame flips to opt-in · **hold** the unbuilt story (DECLARED intent, predictive prompting, and AUTOMATIC model switching — the last is not merely unbuilt but X-01/X-02-forbidden today; `PLAN.md` §0.0 sequences real-time control as *later*). **Proposed DoD addition (founder ruling — changes what "Done" means): a slice is not Done until the public claim surface reflects it.** Precedent exists: T-D1 killed counted-detector claims as a CLASS across nine surfaces once they drifted, and FR-42 is literally "scale claims discipline — measured numbers only". Pin claims to architecture with a test so drift cannot recur (the traceability-gate instrument, applied to claims).
- **API documentation is a machine dump, not documentation (verified 2026-08-01).** `docs-site/api/endpoints.md` (32 KB, generated by `scripts/export_openapi.py`) contains **ZERO fenced code blocks** — no curl, no Python, no TypeScript, nothing runnable. It emits **all 103 OpenAPI paths including HTML pages** (`GET /`, `GET /activity`) into an "API" reference, so a developer cannot tell what is actually callable; it documents the **`x-user-email` dev shim** as a parameter; and its descriptions are internal function docstrings ("Landing.", "Activity Page."), not API prose. Against the Anthropic-docs standard — task-oriented pages, runnable examples per endpoint, explicit auth/errors/pagination, an SDK snippet beside every call — this is a schema dump. Needs: JSON-API-only scope, a worked example per endpoint in curl + Python + TS, auth and error and pagination pages, and a quickstart that runs end-to-end. Interacts with the API-first gap (six bearer-reachable endpoints) — documenting the current surface honestly will make that gap visible, which is a feature not a problem.
- **UI is not enterprise-production grade (founder 2026-08-01).** Recorded as the founder's standing judgement; needs its own audit rather than an agent's guess at what "drastic changes" means. Anchor it to what already exists so it starts from evidence: `docs/design/DESIGN-AUDIT.md` (a completed remediation record), the 41 after-screenshots in `docs/design/evidence/`, the AA-contrast + kit + motion tests that hold today, and LE-11's finding that token PLACEMENT and visual regression are unvalidated. First step should be a founder-led walkthrough naming the specific surfaces that fail the bar, not a redesign proposal.
- **Production drift: shipped changes are not in production (founder 2026-08-01).** `main` moved substantially on 2026-07-31/08-01 (pricing-gate reconciliation, month-end flake, harness fix, R-TRACE registration) and staging auto-deploys on merge, but **prod promotion is founder-gated (LE-2 REDEFINED: staging-auto, prod manual) and has not been dispatched**. So customers are on an older build than the repo describes — which compounds every claim-accuracy item above, because the landing page would then describe a product that is live in staging only. Needs either a promotion or an explicit statement of the intended lag.

- **ADR id collision — nothing allocates ADR numbers (found live 2026-08-01).** The LOOP minted `ADR-8` in PR #118 (pytest-requirements over a hand-rolled marker) while this session minted `ADR-8` for the same decision's build half, independently and simultaneously. Resolved by renumbering the later one to ADR-9, but the cause stands: **ADR ids are allocated by whoever writes next**, with no registry and no uniqueness check — exactly the `R-nn` collision class, now reproduced locally by two authors working in parallel. As the loop ships more autonomously this recurs. Cheap guard: a test asserting every `^ADR-[0-9]+` in docs/02 is unique, the same instrument as the traceability gate.
