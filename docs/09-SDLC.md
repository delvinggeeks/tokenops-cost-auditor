...
| LE-6 observability + kill-switch | loop status + `loop:paused` | **SHIPPED 2026-07-26 · DEFECT FOUND 2026-08-01 (see LE-12)** — kill-switch: the `LOOP_PAUSED` repo variable (`gh variable set LOOP_PAUSED --body true`) halts ALL auto-merge instantly, honored in auto-merge.yml's guard. Observability: `scripts/loop_status.py` reports paused / auto-merge / gate-round-required / armed-PRs / gate-round pass rate in one screen. |
| **LE-7 requirement-bound tests** | marker carries the FR/NFR id | **NOT BUILT** — registered 2026-07-31 (R-TRACE, CHG below). Every test declares the requirement(s) it verifies via **`pytest-requirements`** (BSD-3, pytest-only dependency — ADOPTED per ADR-8, probed not assumed: the marker self-registers, emits `requirement_id` into JUnit XML, and a five-line collection hook yields the full map without running the suite). `@pytest.mark.verifies_requirement("FR-07")` becomes the SINGLE source of the requirement↔test edge, so `docs/04` becomes DERIVED rather than authored. Includes backfilling the current `tests/` tree. A marker naming an id absent from `docs/01` fails collection. |
