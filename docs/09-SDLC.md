...
| **LE-7 requirement-bound tests** | marker carries the FR/NFR id | **NOT BUILT** — registered 2026-07-31 (R-TRACE, CHG below). Every test declares the requirement(s) it verifies via a registered pytest marker; that marker becomes the SINGLE source of the requirement↔test edge, so `docs/04` becomes DERIVED rather than authored. Includes backfilling the current `tests/` tree. A marker naming an id absent from `docs/01` fails collection. |
