...
| R-D6-AGG session aggregation (founder 2026-07-18) | D6 and D4 emit ONE finding per session (tag split at gaps > D6_SESSION_GAP_S=900s), impact summed over qualifying runs/clusters; evidence sampled across constituents (≤20); per-run/cluster breakdown in Finding.detail → report.json. GOLDEN DERIVATION UNCHANGED BY CONSTRUCTION: waste_pack D4 block = 5 calls 20s apart = one cluster in one session → 1 finding, 4×0.001275×10 = 0.0510 exactly; waste_pack D6 block = 12 calls 65s apart, max gap 65s < 900s = one session containing run1(n=10, fires) + run2(n=2, silent) → 1 finding = 8×1200×$1/1e6×10 = 0.096 exactly. Suite re-verified green same commit | d4_retry_storm.py, d6_chatty_loop.py, findings.py |
