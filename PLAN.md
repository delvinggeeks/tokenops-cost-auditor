...
Author model: main thread. Date: 2026-07-17.
...
**R-Q1/Q2 GATE CADENCE — founder rule adopted, one nuance flagged.** My original
proposal (gates at the end of *each* Dn in a grouped row) is WITHDRAWN. Adopted rule:
gates fire only at milestone-group boundaries — grouped rows gate ONCE, at the end of
the group. ux-reviewer window = D7–D9 inclusive; architect UML emission at D6 and D13
only. **Flagged nuance awaiting confirmation at the D1 stop:** under group gating no
gate fires at the end of D6, so the architect's "D6" UML emission mechanically happens
at the D6-D7 group gate (end of D7), generated from the D6 runner architecture. If you
want a dedicated architect-only invocation at end of D6 instead, say so; otherwise the
end-of-D7 emission stands. Resolved schedule in §2.
...
**MYPY scope**: strict on `src/tokenops/services/*` per docs/05 §4; standard elsewhere.
...
- `src/tokenops/__init__.py`, `config.py` (pydantic-settings; every var in docs/03 §7),
  `main.py` (app factory, request-id middleware, `/healthz` with db + disk_free checks)
- `src/tokenops/obs/{logging.py,errors.py,ratelimit.py}` (structlog JSON, env-gated
...
- `src/tokenops/persistence/models.py` (DeclarativeBase only), alembic init
...
- `cli.py` — `tokenops audit file.jsonl --out report.pdf` (FR-04)
